...
- 可手动选择扬声器、耳机等系统输出设备，也可让程序自动选择。
- 启动前自动检查模型文件和 GPU，并可用 2 秒声音检测确认设备选对了。
- 英语、俄语或自动语言识别。
- 英文/俄文与中文双语置顶悬浮字幕。
...
双击仓库根目录的 `启动字幕工具.cmd`。等顶部的运行文件和 GPU 自检完成后：

1. 选择正在播放视频的系统输出设备；不确定时保持“自动选择”。
2. 播放视频，点击“检测声音”；音量条变绿即表示可以听到系统声音。
3. 选择视频语言，点击“开始字幕”。

声音检测只在内存中分析 2 秒音量，不会保存录音。字幕窗口会显示在屏幕底部。
